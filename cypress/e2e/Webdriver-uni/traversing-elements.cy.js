...
describe("Traversing DOM elements in Cypress", () => {
  beforeEach(() => {
    ...
    it("eq() to retrieve a specific element based on index", () => {
      cy.get('.traversal-drinks-list').find('li').eq(2).should('contain', 'Milk')
    });
  
    it("filter() to retrieve DOM elements that match a specific selector", () => {
      cy.get('button').filter('.active').should('contain', 'Button-1')
    ...
    it("nextAll() to get all of the next sibling DOM elements within elements", () => {
      cy.get('.traversal-drinks-list').find('li').eq(2).nextAll().should('have.length', 2)
    });
  
    it("nextUntil() to get all of the next sibling DOM elements within elements until another element", () => {
      cy.get('.traversal-drinks-list').find('li').first().nextUntil('#milk')
    });
    ...
    it("prevUntil() to get all previous sibling DOM elements within elements until other element", () => {
      cy.get('#veggie').prevUntil('#fruits').should('match', 'li')
    });
  
    it.only("siblings() To get all sibling DOM elements of elements", () => {
      cy.get('.btn-group-toggle').find('button').filter('.active').siblings().should('have.length', 3)
    });
});
